...
BEGIN
  ...
  IF v_subpart_count > 0 THEN
    FOR rec IN (
      SELECT 'ALTER TABLE ' || table_owner || '.' || table_name ||
             ' MOVE SUBPARTITION ' || subpartition_name || ' UPDATE INDEXES ' AS qry
      FROM dba_tab_subpartitions
      WHERE table_owner = '&v_schema' AND table_name = '&v_table'
    ) LOOP
      v_qry := rec.qry;
  ...
  FOR rec IN (
    ...
    v_qry := rec.qry;
    DBMS_OUTPUT.PUT_LINE('SP_INDX: ' || v_qry);
    EXECUTE IMMEDIATE v_qry;
    DBMS_OUTPUT.PUT_LINE('Rebuilding subpartition: ' || TO_CHAR(SYSTIMESTAMP, 'DD-MM-YYYY HH24:MI:SS'));
  END LOOP;
