...
def find_oracle_homes(ssh):
    ...
    if not unique_homes:
        unique_homes.append("/u01/app/oracle/product/19.0.0/db_1")
...
def get_oracle_patches(ssh):
    ...
    for oracle_home in oracle_homes:
        ...
        # Run OPatch and sqlplus commands
        lspatches_cmd = f"{oracle_home}/OPatch/opatch lspatches"
        version_cmd = f"{oracle_home}/OPatch/opatch version"
        oracle_version_cmd = f"/bin/bash -lc '{oracle_home}/bin/sqlplus -V 2>/dev/null'"
        ...
        # print(f"Oracle version output: {oracle_version_output}")  # ✅ Make sure this prints

        # Get SID
        sid_cmd = "ps -eo args | grep '[o]ra_pmon_' | grep -v '+ASM' | awk -F_ '{print $NF}'"
        sid_output = run_command(ssh, sid_cmd).strip()

        # If no SID found, try to infer
        if not sid_output:
        ...
        sid_list = sid_output.splitlines() or ['unknown']

        # Get lsinv output with applied patches and dates
        lsinv_cmd = f"{oracle_home}/OPatch/opatch lsinv | grep applied"
        lsinv_output = run_command(ssh, lsinv_cmd)

        # Extract all dates from lsinv_output
        dates = re.findall(r"applied on (.+)", lsinv_output)
        latest_patch_date = None

        from datetime import datetime as dt

    def parse_date(date_str):
        # Example date format: Thu Sep 14 11:04:27 IST 2023
            try:
            # Remove timezone for parsing since strptime may not handle 'IST'
                parts = date_str.split()
                if len(parts) == 6:
                # Remove the timezone part (index 4)
                    date_str_mod = " ".join(parts[:4] + parts[5:])
                else:
                    date_str_mod = date_str
                return dt.strptime(date_str_mod, "%a %b %d %H:%M:%S %Y")
    
            except Exception:
                return None

    parsed_dates = [parse_date(d) for d in dates if parse_date(d) is not None]

    if parsed_dates:
        latest_patch_date = max(parsed_dates)
    else:
        latest_patch_date = None

# Store this in the patch info dictionary along with other info
    ...
    for sid in sid_list:
        patch_results.append({
                ...
                "latest_patch_date": latest_patch_date.strftime("%Y-%m-%d %H:%M:%S") if latest_patch_date else ""
    })
...
def parse_patch_info(patch_info):
    ...
    results["latest_patch_date"] = patch_info.get("latest_patch_date", "")

    # Extract Oracle version information if available
    oracle_version_match = re.search(r'Version (\d+\.\d+\.\d+\.\d+(?:\.\d+)?)', patch_info["oracle_version"])
...
def export_to_excel(server_data, filename):
    """
    Exports the collected data to an Excel file with styled and centered headers.
    """
    wb = Workbook()
    ws = wb.active
    ...
    header_fill = PatternFill(start_color="228B22", end_color="228B22", fill_type="solid")  # ForestGreen
    header_alignment = Alignment(horizontal="center", vertical="center")

    # Write headers
    headers = [
        "Hostname", "SID", "Oracle Home", "Oracle Version", "OPatch Version", 
        "Database Release", "OJVM RELEASE", "OCW RELEASE", "Latest Patch Date"
    ]
    ...
    for server, homes_data in server_data.items():
        for data in homes_data:
            ...
            ws.cell(row=row, column=9, value=data.get("latest_patch_date", ""))
            row += 1

    # Optionally freeze the top row for easier scrolling
    ws.freeze_panes = "A2"
...
def main():
    ...
    server_data = {}
    username = "oracle"
    passwords = ['Ebslabs123', 'imsuser01@123']
    ...
    for server in servers:
        ...
        for pwd in passwords:
            ssh = connect_to_server(server, username, pwd)
            if ssh:
                break
            else:
                print(f"Password '{pwd}' failed for {server}")
